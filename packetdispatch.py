...
def sendUDPpackets(srcAddr,dstAddr, port, requests, size,packetcount):
    source=str(srcAddr) #just in case to convert to string... it can probably be removed and srcAddr can be used instead
...
#TCP(flags="S", sport=sport, dport=int(dport), options=[('Timestamp',(0,0))], chksum = 0)
    ...
    send_mod(pckt,loop=1,port_random_flag=port_random,size_random_flag=size_random,src_random_flag=src_random,specific_subnet_random=subnet_random,source_addr=source,count=targ_reqs,pcktcount=packetcount)
    return

def sendTCPpackets(srcAddr,dstAddr, port, requests, size,flags,packetcount):
    source=str(srcAddr) #just in case to convert to string... it can probably be removed and srcAddr can be used instead
    targ_addr = str(dstAddr)
    targ_port = int(port)
    targ_reqs = int(requests)
...
def sendICMPpackets(srcAddr,dstAddr,requests,size,icmp_rand,packetcount):
    source=str(srcAddr) #just in case to convert to string... it can probably be removed and srcAddr can be used instead
    targ_addr = str(dstAddr)
    targ_reqs = int(requests)
    packet_size = int(size)
    ...
    #implement requests
    #print(str(subnet_random))
    send_mod(pckt,loop=1,src_random_flag=src_random,specific_subnet_random=subnet_random,source_addr=source,count=targ_reqs,size_random_flag=size_random,pcktcount=packetcount)
    return
