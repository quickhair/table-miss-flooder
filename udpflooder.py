...
def main():
    ...
    # input_parser.add_argument("-chk", "--checksum", type=int, nargs=1, default=DEFAULT_CHKSUM, help="Set a checksum value")

    # TCP FLAG
    input_parser.add_argument("-f", "--flag", type=str, nargs=1, default=DEFAULT_TCP_FLAG,
                              help="Set a TCP flag. Options: S(YN), A(CK), F(IN), U(RG), P(SH), R(ST),E(CE),C(WR),N(S)[NS is an experimental flag,see RFC 3540]")
    ## ICMP TYPE -not implemented-
    ## input_parser.add_argument("-t", "--type", type=int, nargs=1,default=DEFAULT_ICMP_CODE, help="Specify ICMP packet type. ICMP TYPES and CODES: https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml")
    ## ICMP CODE -not implemented-
    ## input_parser.add_argument("-c", "--code", type=int, nargs=1,default=DEFAULT_ICMP_CODE, help="Specify ICMP packet type. ICMP TYPES and CODES: https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml")
    ## ICMP RAND TYPE & CODE
    input_parser.add_argument("-rtc", "--randomtypecode", type=int, nargs=1,default=DEFAULT_ICMP_TYPE_FLAG,
                              help="Randomly generates ICMP packets of different type. 1 for ON, 0 for OFF. ICMP TYPES and CODES: https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml")
    ...
    try:
        if (protocol == "UDP"):
            packetdispatch.sendUDPpackets(srcAddr, dstAddr, port, requests, size,packetcount)
        elif (protocol == "TCP"):
            packetdispatch.sendTCPpackets(srcAddr, dstAddr, port, requests, size, flags,packetcount)
        elif(protocol=="ICMP"):
            packetdispatch.sendICMPpackets(srcAddr, dstAddr, requests, size, icmp_rand,packetcount)
        else:
