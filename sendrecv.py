...
def _generatePacket(size):
    ...
    return packet
'''
def _generateICMPtype():
	#open file, load everything to a list, then assign randomly through each iter
	csv_file=open('icmp-parameters-types.csv')
        ...
        while loop:
            ...
            for p in x:
                ...
                if (port_random_flag):
                    ...
                    p.dport = targ_port
                if (size_random_flag):
                    packet_size = int(random.randrange(0,
                                                       6550))  # 7 I removed 7 because there seems to be some boundary crossing(bleeding)
...
send(packets, [inter=0], [loop=0], [count=None], [verbose=conf.verb], [realtime=None], [return_packets=False],  # noqa: E501
    ...
    results = __gen_send_mod(socket, x, inter=inter, loop=loop,
